...
El Excel de Toto (`docs/Cartas KG.xlsx` y el `docs/Cartas KG.csv` que sale de él, **los dos ignorados por git a propósito: son sus ideas sin publicar, no se versionan ni se suben a ninguna parte**) es la fuente de la MECÁNICA, nunca de la REDACCIÓN.

**Cuando Toto toque el Excel no hay que hacer nada por su parte**: él lo edita donde siempre (Dropbox) y aquí se ejecuta `python3 tools/excel-a-csv.py`, que **se lo descarga** con el enlace privado de `docs/.cartas-kg.url` y regenera el CSV recortando **de la columna K en adelante** (notas suyas que no deben salir). No instala nada: un `.xlsx` es un ZIP con XML y Python trae las dos cosas. **Si el CSV parece viejo o falta una carta que Toto dice haber añadido, lo primero es ejecutarlo.** El enlace, el `.xlsx` y el CSV están los tres en `.gitignore` y no salen del servidor jamás. Copiar su texto tal cual es el error más repetido de esta fase; el CSV arrastra formulaciones viejas de antes de fijar la rúbrica.
