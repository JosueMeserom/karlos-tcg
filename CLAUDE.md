...
**Cuando Toto toque el Excel**: él guarda el `.xlsx` encima del que hay en `docs/` (desde Windows, por Z:) y se regenera el CSV con `python3 tools/excel-a-csv.py`. El script recorta **de la columna K en adelante** (notas privadas suyas) sin depender de que nadie se acuerde de borrarlas a mano, y no necesita instalar nada: un `.xlsx` es un ZIP con XML y Python trae las dos cosas. Si el CSV parece viejo o falta una carta que Toto dice haber añadido, lo primero es ejecutarlo. Copiar su texto tal cual es el error más repetido de esta fase; el CSV arrastra formulaciones viejas de antes de fijar la rúbrica.
